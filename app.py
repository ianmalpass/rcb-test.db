...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        st.sidebar.title("AI-sistant Navigation")
        ...
        # --- ANALYTICS ---
            ...
            if not p_df.empty:
                ...
                st.subheader("Temperature & Toluene Trends")
                fig = go.Figure()
        ...
        # --- HELP SECTION ---
            ...
            st.title("📖 AI-sistant User Guide")
            with st.expander("🏗️ How to Record a New Bag"):
                st.write("1. Navigate to **Production (Inventory)**.")
                st.write("2. Enter the Lab results (Hardness, Moisture, Toluene, Ash).")
                st.write("3. Verify the weight is **55.0 lbs**.")
                st.write("4. Click **Record Bag**. A label with a barcode will appear.")
                st.write("5. Click **Print Label** to send it to the thermal printer.")

            with st.expander("🚢 How to Ship a Bag"):
                st.write("1. Navigate to **Shipping**.")
                st.write("2. Place cursor in the **Scan Barcode** box.")
                st.write("3. Scan the bag's barcode. The details will automatically populate.")
                st.write("4. Enter the **Customer Name** and click **Ship Bag**.")

            with st.expander("🔥 How to Log Reactor Data"):
                st.write("1. Navigate to **Reactor Logs**.")
                st.write("2. Enter current Toluene, Feed Rate, and Hz settings.")
                st.write("3. Temps default to **500°C** and **550°C** but can be changed if needed.")
                st.write("4. Click **Submit Process Log**.")

            with st.expander("❓ Troubleshooting Barcodes"):
                st.info("If the scanner isn't pulling up the bag: Ensure the cursor is flashing in the 'Scan Barcode' input box before you pull the trigger.")
